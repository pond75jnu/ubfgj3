<%@ Page Title="" Language="C#" MasterPageFile="~/master/master_main.master" AutoEventWireup="true" CodeFile="expenses.aspx.cs" Inherits="staff_expenses" MaintainScrollPositionOnPostback="true" %>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
            ...
            <div class="min-w-0 space-y-6">
                    ...
                    <div class="overflow-x-auto rounded-apple-lg border border-hairline bg-white">                        
                        <asp:GridView ID="gvList" runat="server" DataKeyNames="seq" AllowPaging="false" BorderColor="#dee2e6" CssClass="min-w-[920px] w-full border-collapse text-sm text-ink [&_td]:border-b [&_td]:border-hairline [&_td]:px-4 [&_td]:py-3 [&_th]:border-b [&_th]:border-hairline [&_th]:bg-parchment [&_th]:px-4 [&_th]:py-3 [&_th]:font-semibold"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField DataField="payment_item" HeaderText="지출내용">
                                    <ItemStyle CssClass="nowrap" />
                                    <HeaderStyle CssClass="txt_center nowrap" />
                                </asp:BoundField>
                                <asp:BoundField DataField="payment_format" HeaderText="지출비용">
                                    <ItemStyle CssClass="nowrap txt_right" />
                                    <HeaderStyle CssClass="txt_center nowrap" />
                                </asp:BoundField>
                                <asp:BoundField DataField="payment_dt" HeaderText="지출일자">
                                    <ItemStyle CssClass="nowrap txt_center" />
                                    <HeaderStyle CssClass="txt_center nowrap" />
                                </asp:BoundField>
                                <asp:BoundField DataField="attatch_yn" HeaderText="영수증">
                                    <ItemStyle CssClass="txt_center nowrap" />
                                    <HeaderStyle CssClass="txt_center nowrap" />
                                </asp:BoundField>
                            ...
                            <EmptyDataTemplate>
                                <table class="w-full text-sm text-ink">
                                    <tr>
                                        <td class="px-4 py-6" style="border: 0px none #ffffff;">등록된 지출정보가 없습니다.
                                        </td>
                                    </tr>
                                </table>
                            </EmptyDataTemplate>
                        ...
                        <div class="grid grid-cols-1 gap-2 md:grid-cols-[160px_minmax(0,1fr)] md:items-start">
                            ...
                            <div class="min-w-0">
                                ...
                                <div id="divAttatchImage" runat="server" visible="false" class="space-y-3 pt-3">                                    
                                    <asp:Image ID="AttatchImage" runat="server" CssClass="h-auto max-w-full rounded-apple-sm border border-hairline attatch_image01" AlternateText="영수증" />
                ...
                <div id="divButtons" class="flex flex-wrap gap-2">
                    <a id="btnNew" runat="server" href="/staff/expenses.aspx?mode=write" class="inline-flex min-h-11 items-center justify-center rounded-pill bg-action px-[22px] py-[11px] text-[17px] font-normal text-white transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-action-focus">신규
                    </a>
                    <input type="button" id="btnModify" runat="server" onclick="modify_expenses();" value="수정" class="inline-flex min-h-11 items-center justify-center rounded-pill bg-action px-[22px] py-[11px] text-[17px] font-normal text-white transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-action-focus" />
                    <asp:Button ID="btnSave" runat="server" Text="저장" CssClass="inline-flex min-h-11 items-center justify-center rounded-pill bg-action px-[22px] py-[11px] text-[17px] font-normal text-white transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-action-focus disabled:cursor-not-allowed disabled:bg-[#7a7a7a]" OnClick="btnSave_Click"
                        OnClientClick="return uConfirm_expenses();" />
                    <asp:Button ID="btnDel" runat="server" Text="삭제" CssClass="inline-flex min-h-11 items-center justify-center rounded-pill border border-[#b42318] bg-[#b42318] px-[22px] py-[11px] text-[17px] font-normal text-white transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-action-focus disabled:cursor-not-allowed disabled:border-[#f1b8b2] disabled:bg-[#fef3f2] disabled:text-[#b42318]" OnClientClick="return uConfirmDel_expenses();"
                        OnClick="btnDel_Click" />
                    <a id="btnList" runat="server" href="/staff/expenses.aspx" class="inline-flex min-h-11 items-center justify-center rounded-pill border border-action bg-white px-[22px] py-[11px] text-[17px] font-normal text-action transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-action-focus">목록
                    </a>
                    <a id="btnExportExcel" runat="server" href="javascript:;" onclick="expenses_list_excel();" class="inline-flex min-h-11 items-center justify-center rounded-pill border border-action bg-white px-[22px] py-[11px] text-[17px] font-normal text-action transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-action-focus">
                        엑셀저장
                    </a>
                    <a id="btnPrintDetail" runat="server" href="javascript:;" onclick="expenses_detail_print();" class="inline-flex min-h-11 items-center justify-center rounded-pill border border-action bg-white px-[22px] py-[11px] text-[17px] font-normal text-action transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-action-focus">
                        인쇄
                    </a>
                    <a id="btnPrintAll" runat="server" href="javascript:;" onclick="expenses_all_print();" class="inline-flex min-h-11 items-center justify-center rounded-pill border border-action bg-white px-[22px] py-[11px] text-[17px] font-normal text-action transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-action-focus">
                        지출현황(전체) 인쇄
                    </a>
                </div>
