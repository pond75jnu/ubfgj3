...
public partial class staff_income : System.Web.UI.Page
{
    ...
    protected void LoadCashTypes()
    {
        try
        {
            DataSet ds = EfStoredProcedure.ExecuteDataSet(
                ...
                new SqlParameter("@cash_type", SqlDbType.Int) { Value = 1 },
                new SqlParameter("@exclude_retreat_dues", SqlDbType.Bit) { Value = true });
            ...
            {
                Page.ClientScript.RegisterClientScriptBlock(this.GetType(), "Alert", "<script>alert('수입항목 정보가 없습니다. (관리자 문의)');</script>");
        ...
        {
            Page.ClientScript.RegisterClientScriptBlock(this.GetType(), "Alert", "<script>alert('수입항목 조회 중 에러 발생 : " + Server.HtmlEncode(ex.Message) + @"');</script>");
        }
    }
    ...
    protected void GetList()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                lblSum.Text = "· 총 수입: " + ds.Tables[1].Rows[0]["payment_all_format"].ToString() + " <span style='color:#888888; font-size:0.9rem;'>(수양회비는 실무자가 확인한것만 계산됨)</span>";
            }
            else
                btnExportExcel.Visible = false;
        }
    ...
    {
        decimal _expense = CodeHelper.ParsePositiveWholeWon(hdExpenses.Value, "수입금액");
    ...
    protected void gvList_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            e.Row.Attributes.Add("onclick", "javascript:detailview_income('" + e.Row.Cells[1].Text + "');");
            ...
            if (e.Row.Cells[1].Text.Equals("9999999"))
            {
