...
| `retreat_program.aspx(.cs)` | 활성 수양회 안내 첨부파일을 inline 응답으로 스트리밍 |
...
- 버튼은 `/retreat_program` inline 응답을 모달 iframe으로 표시하며, 같은 URL을 새 탭으로 열 수도 있다.
...
| --- | --- | --- | --- | --- |
| 공개 수양회 안내 | `Default.aspx`, `retreat_program.aspx` | `retreat_master`, `retreatdues_master` | `SP_retreat_active_info_sel`, `SP_retreat_dues_by_retreat_sel`, `SP_retreat_active_file_sel` | 활성 수양회와 회비, 안내 PDF inline 표시 |
...
- 요회명을 누르면 구성원별 선택·미선택 상태를 모달에서 확인한다.
- 두 번째 `사전설정` 탭은 수양회 날짜를 열, 아침·점심·저녁을 행으로 출력한다. 저장된 설정이 없으면 첫날 저녁, 중간 날짜 전체 식사, 마지막 날 아침·점심을 기본 제공하며 하루짜리 수양회는 세 끼를 기본 제공한다.
- 설정 변경으로 기존 선택과 충돌하면 일반 저장을 막고 경고한다. 사용자가 강제 저장을 선택한 경우 새 설정에서 제공하지 않는 식사의 기존 선택을 정리한다.
...
- 활성 수양회의 전체 활성 요회를 조회하고 구성원 한 명을 한 행으로 출력한다. 구성원이 없는 요회도 `(구성원 없음)` 행으로 남긴다.
- 고정 열은 `요회`, `제출상태`, `성명`, `회원구분`이고, 이후 열은 사전설정에서 제공되는 날짜·식사만 사용한다. `마지막저장`과 미제공 식사 열은 출력하지 않는다.
- 선택한 식사는 숫자 `1`, 선택하지 않은 식사는 숫자 `0`이며 마지막에 `선택합계`를 출력한다.
...
- 식사 상세 엑셀에 전체 요회가 포함되는지, 미제공 식사와 `마지막저장` 열이 제외되는지, 선택값이 숫자 `1/0`이고 열 너비가 내용에 맞는지 확인한다.
