...
2026-06-28 기준으로 완료된 마이그레이션과 UI 정비 내역이다.
...
- 현재 공통 CSS/JS 캐시 버전은 `tailwind.css?v=tailwind-02`, `custom.css?v=tailwind-40`, `custom.js?v=tailwind-19`이다.
...
- 홈 CTA `프로그램 세부보기` 버튼은 공통 compact button 크기와 맞췄다.
...
| 경로 | 역할 |
| --- | --- |
| `Default.aspx(.cs)` | 공개 메인. 활성 수양회 안내, 장소/기간, 회비 구분, 첨부파일 다운로드 |
...
- `retreat_master.file_nm`, `file_data`가 있으면 첨부파일 다운로드 버튼을 활성화한다.
...
- 엑셀 저장은 관리자/실무자만 가능하며 GridView HTML을 `.xls`로 내려보낸다.
...
- 상세 인쇄: `in_ex_detail_print.aspx?seq={payment_seq}&type=1|2`
- 전체 인쇄: `in_ex_all_print.aspx?ret={retreat_seq}&type=1|2`
- 엑셀: `in_ex_excel_export.aspx?ret={retreat_seq}&type=1|2`
...
- 요회명 caption은 해당 요회의 `group/usermanage.aspx`로 연결된다.
...
| --- | --- | --- | --- | --- |
| 공개 수양회 안내 | `Default.aspx` | `retreat_master`, `retreatdues_master` | `SP_retreat_active_info_sel`, `SP_retreat_dues_by_retreat_sel`, `SP_retreat_active_file_sel` | 활성 수양회와 회비, 첨부파일 표시 |
...
| 인쇄/엑셀 | `staff/*print.aspx`, `staff/*export.aspx` | `payment_master`, `group_members` | `SP_payment_print_detail_get`, `SP_income_get_list`, `SP_registatus_excel_get_list` | GridView HTML 기반 `.xls` 다운로드 |
...
| 기능 | 파일 | 확인할 DB/의존성 |
| --- | --- | --- |
| 메인 수양회 안내 | `Default.aspx.cs` | `retreat_master`, `retreatdues_master`, `SP_retreat_*`, `SP_retreat_dues_by_retreat_sel` |
...
| 엑셀/인쇄 | `staff/*export.aspx.cs`, `staff/*print.aspx.cs` | GridView HTML export, no-frame master |
...
3. 페이지 제목/breadcrumb를 쓰려면 `menu_master.menu_path`에 실제 URL을 등록한다.
4. `menu_master.menu_auth`를 `anonymous`, `user`, `manager`, `admin` 중 하나로 설정한다.
5. DB 접근은 `EfStoredProcedure`와 `SqlParameter`로 Stored Procedure를 호출한다.
6. 인라인 SQL 문자열을 새로 추가하지 않는다.
7. 신규 업무 테이블에는 가능하면 기존 감사 컬럼 패턴을 따른다.
8. 클라이언트 검증이 필요한 경우 `common/js/custom.js`에 함수가 추가되는지 확인한다.
9. 수입/지출, 실무확인, 현황에 영향을 주는 변경은 관련 프로시저까지 같이 확인한다.
...
- 엑셀 출력은 HTML 기반 `.xls`이므로 브라우저/Office 보안 경고가 날 수 있다.
...
| --- | --- |
| `/` | 활성 수양회 안내 |
| `/member/login.aspx` | 로그인 |
| `/member/join.aspx` | 회원가입 |
| `/info/modify01.aspx` | 내 정보 수정 |
| `/manage/members.aspx` | 회원 관리 |
| `/manage/groups.aspx` | 요회 관리 |
| `/group/usermanage.aspx` | 요회 구성원·회비 관리 |
| `/staff/retreat.aspx` | 수양회 관리 |
| `/staff/retreatdues.aspx` | 수양회비 구분 관리 |
| `/staff/registatus.aspx` | 실무자 등록 확인 |
| `/staff/income.aspx` | 수입 관리 |
| `/staff/expenses.aspx` | 지출 관리 |
| `/staff/items.aspx` | 수입/지출 코드 관리 |
| `/staff/status.aspx` | 종합 현황 |
